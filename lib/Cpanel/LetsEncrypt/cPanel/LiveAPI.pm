...
sub listaccts {
    ...
    push(@domains, $account_ref->{'main_domain'}) if ! $ssl_vhosts->{$account_ref->{'main_domain'}};

    push(@alt_domains, @{$account_ref->{'addon_domains'}});
    push(@alt_domains, @{$account_ref->{'sub_domains'}});

    foreach my $alt_domain (@alt_domains) {
        my $main_domain = $self->{api}->uapi('DomainInfo', 'single_domain_data', {'domain'    => $alt_domain })->{'cpanelresult'}->{'result'}->{'data'}->{'servername'};
...
sub get_domain_userdata {
    ...
    my $domainuserdata = $self->{'api'}->uapi('DomainInfo', 'single_domain_data', { 'domain' => $domain });
...
sub install_ssl_certificate {
    ...
    my $status = $self->{'api'}->uapi( 'SSL', 'install_ssl', {
